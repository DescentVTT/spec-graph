 ...
 *
 * `~=` is the one predicate in the selector language that used to hand a
 * user-supplied string to `RegExp` and wait. That is tolerable at a prompt,
 * where a human can press Ctrl-C. It is not tolerable in `.spec-graph.json`,
 * where the same string runs unattended in CI against every node in the corpus.
 *
 ...
 * ```
 *
 * Each word of k letters can be cut into pieces 2^(k-1) ways, and the engine
 * tries the product of those, which is why adding one five-letter word costs
 * another factor of sixteen. A sixty-character title does not finish this year.
 *
 ...
 * most once per position, which makes the work exactly O(pattern x subject):
 * the right-hand column above, growing with the subject and not with the
 * pattern's ambiguity.
 *
 ...
 * with a message and an offset, rather than accepted and then slow. That is the
 ...
 * clause was not enough; running it was. See ADR-0017.
 */
...
class PatternParser {
  ...
  parse(): Node {
    ...
    if (this.position < this.source.length) throw new PatternError('unmatched ")"', this.position);
    return node;
  }
  ...
  private parseBraces(): { min: number; max: number } | null {
    ...
    if (min > max) throw new PatternError('numbers out of order in a counted repetition', this.position);
  ...
  private parseAtom(): Node {
    ...
    switch (char) {
      case undefined:
        throw new PatternError('pattern ends where an expression was expected', start);
      case '(':
      ...
      case '*':
      case '+':
        throw new PatternError(`nothing to repeat before "${char}"`, start);
      case '\\':
  ...
  private parseGroup(): Node {
    ...
    if (this.source.startsWith('?:', this.position)) {
      ...
      if (named) {
        ...
        throw new PatternError('unsupported group: expected (?:...) or (?<name>...)', start);
      }
    }

    const inner = this.parseAlternation();
    if (this.peek() !== ')') throw new PatternError('unmatched "("', start);
    this.position += 1;
    return inner;
  }
  ...
  private parseClass(): CharSet {
    ...
    while (this.position < this.source.length && this.peek() !== ']') {
      ...
      if (typeof low === 'number' && typeof high === 'number') {
        if (low > high) throw new PatternError('characters out of order in a character class', dash);
    ...
    if (this.peek() !== ']') throw new PatternError('unterminated character class', start);
    this.position += 1;
  ...
  private parseClassAtom(): number | readonly Range[] {
    ...
    if (this.source[this.position + 1] === 'B') {
      throw new PatternError('"\\B" is an assertion, not a character, so it cannot be in a class', this.position);
    }
  ...
  private parseCharacterEscape(): CharSet {
    ...
    this.position += 1;
    const char = this.peek();
    if (char === undefined) throw new PatternError('pattern ends with a backslash', start);
    this.position += 1;
    ...
    switch (char) {
      ...
      case '0':
        if (/^\d/.test(this.source.slice(this.position))) {
          throw new PatternError('an octal escape is not supported - write \\xHH or \\uHHHH', start);
        }
        return literal(0);
      case 'x':
      ...
      case 'c': {
        ...
        if (letter === undefined || !/[A-Za-z]/.test(letter)) {
          throw new PatternError('expected a letter after "\\c"', start);
        }
        this.position += 1;
      ...
      default:
        ...
        if (/[A-Za-z]/.test(char)) {
          ...
          throw new PatternError(`unknown escape "\\${char}"`, start);
        }
  ...
  private readHex(digits: number, start: number): number {
    ...
    if (raw.length !== digits || !/^[0-9a-fA-F]+$/.test(raw)) {
      throw new PatternError(`expected ${digits} hex digits after "\\${this.source[start + 1] as string}"`, start);
    }
