...
describe('mergeReports', () => {
  it('merges the shards back into the sweep they were split from', () => {
    ...
    expect(merged.projectRoot).toBe('/work/spec-graph');
  });
...
describe('this repository', () => {
  ...
  it('runs every test in every shard', () => {
    ...
    expect(config).toContain("vitest: { configFile: 'vitest.mutation.config.ts', related: false }");
  });
