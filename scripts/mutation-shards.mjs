/**
 * The hosted mutation sweep, split across parallel CI jobs and put back
 * together as one report with one score.
 *
 * One runner took 166 to 171 minutes for the sweep, three times, on identical
 * source (ADR-0007). ADR-0019 has the measurements behind the split and behind
 * the table below.
 *
 ...
 * does.
 *
 * Adapted from @descent-vtt/spec-guard, where the same merge has scored every
 * sweep since its 0.9.0.
 *
...
// Stryker keeps the verdicts an incremental file holds for files a run does not
// mutate, so that a partial run does not forget them - and reports them as its
// own. A shard started from the whole sweep's file would report every other
// shard's files too, with the last sweep's verdicts. Each shard starts from its
// own part: every test, and only the files it mutates.
